...
## Виджет: рейтинг питомцев

Партиал, встраивается в шаблон ленты (left_sidebar), не отдельная страница.

### Контекст (FeedView.get_context_data)
| Переменная | Тип | Описание |
| --- | --- | --- |
| pet_rating | QuerySet[Pet] | топ-10 питомцев по сумме реакций+комментов ко всем постам за 7 дней; annotate: rating; питомцы с rating=0 не попадают |

Вызов: `{% include "includes/rating_widget.html" with pet_rating=pet_rating only %}`

- Рейтинг глобальный, от подписок не зависит
- Пустой топ (нет активности за неделю) -> подсказка, не ошибка
- Критерий зафиксирован: сумма реакций+комментов;
